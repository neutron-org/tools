...
AIRDOP_CONTRACT_ADDRESS=neutron198sxsrjvt2v2lln2ajn82ks76k97mj72mtgl7309jehd0vy8rezs7e6c56
...
UPDATE_RESERVE_MSG_BASE64=$(json_to_base64 "$UPDATE_RESERVE_MSG")

WITHDRAW_MSG='{
        "withdraw_all": {}
}'

WITHDRAW_MSG_BASE64=$(json_to_base64 "$WITHDRAW_MSG")
...
MIGRATE_MSGS='[
    {
        "wasm": {
            "execute": {
                "contract_addr": "'"${AIRDOP_CONTRACT_ADDRESS}"'",
                "msg": "'"${UPDATE_RESERVE_MSG_BASE64}"'"
            }
        }
    },
    {
    ...
    },
    {
        "bank": {
          "send": {
            "to_address": "'"${AIRDOP_TRANSFER_CONTRACT_ADDRESS}"'",
            "amount": [
              {
                "denom": "untrn",
                "amount": "42727949222050"
              }
            ]
          }
        }
