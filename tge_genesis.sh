...
# ACCOUNTS
# -------------------------------------------
LTI_START_TIMESTAMP=1715335200
...
VESTING_LP_VAULT_BINARY=$DAO_CONTRACTS_BINARIES_DIR/vesting_lp_vault.wasm
...
VESTING_LP_VAULT_BINARY_ID=$(store_binary "$VESTING_LP_VAULT_BINARY" "$NEUTRON_DAO_ADDRESS")
...
LP_VESTING_VAULT_CONTRACT_ADDRESS=$(generate_contract_address $VESTING_LP_VAULT_BINARY_ID) && ((INSTANCE_ID_COUNTER++))
...
instantiate_contract $ASTROPORT_SATELLITE_CONTRACT_BINARY_ID "$ASTROPORT_SATELLITE_INIT_MSG" "astroport.satellite" "$ASTROPORT_MULTISIG_ADDRESS"
instantiate_contract $ASTROPORT_NATIVE_COIN_REGISTRY_CONTRACT_BINARY_ID "$ASTROPORT_NATIVE_COIN_REGISTRY_INIT_MSG" "astroport.native_coin_registry" "$ASTROPORT_SATELLITE_CONTRACT_ADDRESS"
execute_contract $ASTROPORT_NATIVE_COIN_REGISTRY_CONTRACT_ADDRESS "$SET_UNTRN_PRECISION_MSG" "$ASTROPORT_MULTISIG_ADDRESS"
instantiate_contract $ASTROPORT_FACTORY_CONTRACT_BINARY_ID "$ASTROPORT_FACTORY_INIT_MSG" "astroport.factory" "$ASTROPORT_SATELLITE_CONTRACT_ADDRESS"
...
ADD_LP_VESTING_VAULT_MSG='{
  "add_voting_vault": {
    "new_voting_vault_contract": "'"$LP_VESTING_VAULT_CONTRACT_ADDRESS"'"
  }
}'
ADD_INVESTORS_VAULT_MSG='{
  "add_voting_vault": {
    "new_voting_vault_contract": "'"$INVESTORS_VESTING_VAULT_CONTRACT_ADDRESS"'"
  }
}'
...
instantiate_contract $LOCKDROP_CONTRACT_BINARY_ID "$LOCKDROP_INIT_MSG" "tge.lockdrop" "$NEUTRON_DAO_ADDRESS"
...
instantiate_contract $AUCTION_CONTRACT_BINARY_ID "$AUCTION_INIT_MSG" "tge.auction" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $CREDITS_CONTRACT_BINARY_ID "$CREDITS_INIT_MSG" "tge.credits" "$NEUTRON_DAO_ADDRESS"
...
instantiate_contract $AIRDROP_CONTRACT_BINARY_ID "$AIRDROP_INIT_MSG" "tge.airdrop" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $PRICE_FEED_CONTRACT_BINARY_ID "$PRICE_FEED_INIT_MSG" "tge.price_feed" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $TWAP_ORACLE_CONTRACT_BINARY_ID "$USDC_TWAP_INIT_MSG" "astroport.usdc_twap_oracle" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $TWAP_ORACLE_CONTRACT_BINARY_ID "$ATOM_TWAP_INIT_MSG" "astroport.atom_twap_oracle" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $LP_VESTING_CONTRACT_BINARY_ID "$USDC_LP_VESTING_INIT_MSG" "vesting.usdc_lp" "$NEUTRON_DAO_ADDRESS"
instantiate_contract $LP_VESTING_CONTRACT_BINARY_ID "$ATOM_LP_VESTING_INIT_MSG" "vesting.atom_lp" "$NEUTRON_DAO_ADDRESS"
...
instantiate_contract $VESTING_INVESTORS_CONTRACT_BINARY_ID "$VESTING_INVESTORS_INIT_MSG" "vesting.investors" "$TOKEN_ISSUER_MULTISIG_ADDRESS_2"
instantiate_contract $VESTING_INVESTORS_CONTRACT_BINARY_ID "$VESTING_INVESTORS_INIT_MSG" "vesting.investors_without_voting_power" "$TOKEN_ISSUER_MULTISIG_ADDRESS_2"
instantiate_contract $VESTING_LTI_CONTRACT_BINARY_ID "$VESTING_LTI_INIT_MSG" "vesting.lti" "$FOUNDATION_MULTISIG_ADDRESS"
...
instantiate_contract $VESTING_LP_VAULT_BINARY_ID "$LP_VAULT_INIT_MSG" "neutron.voting.vaults.lp_vesting" "$NEUTRON_DAO_ADDRESS"
...
execute_contract "$NEUTRON_VOTING_REGISTRY_CONTRACT_ADDRESS" "$ADD_INVESTORS_VAULT_MSG" "$NEUTRON_DAO_ADDRESS"
execute_contract "$NEUTRON_VOTING_REGISTRY_CONTRACT_ADDRESS" "$ADD_LP_VESTING_VAULT_MSG" "$NEUTRON_DAO_ADDRESS"
...
echo "LP_VESTING_VAULT_CONTRACT_ADDRESS:" $LP_VESTING_VAULT_CONTRACT_ADDRESS
